package otus.nchuraeva.spring.task13;
...
import otus.nchuraeva.spring.task13.controller.SubscriptionController;
import otus.nchuraeva.spring.task13.document.Category;
import otus.nchuraeva.spring.task13.document.Service;
...
public class SubscriptionControllerTest {
    ...
    @Test
    public void testShowAllSubscriptionPage() throws Exception {
        given(subscriptionService.findAllSubscriptions()).willReturn(Collections.singletonList(subscription));
        List<Subscription> exceptedSubscriptions = Collections.singletonList(subscription);

        mockMvc.perform(get("/allSubscriptions"))
                .andDo(print())
                .andExpect(view().name("subscriptions"))
                .andExpect(model().attribute("subscriptions", exceptedSubscriptions))
                .andExpect(status().isOk());
